...
export async function syncFootballData(options: {
  ...
  try {
    ...
    for (const row of standingRows) {
      const team = teamIds.get(row.team.id) ?? (await upsertTeam(row.team));
      teamIds.set(row.team.id, team);
    ...
    await prisma.syncRun.update({
      ...
      data: {
        status: "SUCCEEDED",
        finishedAt: new Date(),
        fetchedCount: matches.length + standingRows.length,
        upsertedCount: matches.length + standingRows.length,
